...
このディレクトリと `MyDirectXRenderer/RenderGraph/` は **UTF-8**（本体の他のファイルは Shift-JIS）。
本体に置いたファイルは **BOM 付き** にしてあり、MSVC がファイルごとに判定してくれるので共存できる。
**プロジェクトに `/utf-8` は付けないこと**（付けると Shift-JIS 側が壊れる）。
このディレクトリの `main.cpp` は Visual Studio で開かない前提なので BOM なしのままでよい。
