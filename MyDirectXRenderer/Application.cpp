...
#include "Debug.h"
#include "RenderGraph/Dx12/Dx12CommandContext.h"
#include "RenderGraph/Dx12/Dx12ResourceAllocator.h"
#include "RenderGraph/RenderGraph.h"
#include "RenderGraph/TexturePool.h"
...
bool Application::Init() {
	...
	if (!_hwnd) return DebugFail("Application::Init", "ウィンドウの生成");
	...
	if (!_peraRenderer->Init(*_allocator))   // パイプライン構築
		return DebugFail("Application::Init", "PeraRenderer::Init");

	// PMD
	_pmdRenderer.reset(new PMDRenderer(*_dx12));
	if (!_pmdRenderer->Init())               // パイプライン構築
	...
	if (_pmdActor->Load("Model/初音ミク.pmd")) {
		...
		// モデルが無いのは失敗ではないが、黙って消えると気付けないので残しておく
		::OutputDebugStringA("[INFO] PMD モデルを読み込めなかったので表示をスキップします\n");
