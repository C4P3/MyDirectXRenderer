...
bool Dx12Wrapper::Init(HWND hwnd, int window_width, int window_height)
{
...
#endif
	if (FAILED(result)) return DebugFail("Dx12Wrapper::Init", "DXGI ファクトリーの生成", result);
	...
	if (_dev == nullptr)
		return DebugFail("Dx12Wrapper::Init", "D3D12 デバイスの生成（対応する機能レベルが無い）");

	// コマンドアロケーター
	result = _dev->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS(&_cmdAllocator));
	if (FAILED(result)) return DebugFail("Dx12Wrapper::Init", "コマンドアロケーターの生成", result);

	// コマンドリスト
	result = _dev->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, _cmdAllocator.Get(), nullptr, IID_PPV_ARGS(&_cmdList));
	if (FAILED(result)) return DebugFail("Dx12Wrapper::Init", "コマンドリストの生成", result);
	...
	if (FAILED(result)) return DebugFail("Dx12Wrapper::Init", "コマンドキューの生成", result);
	...
	if (FAILED(result)) return DebugFail("Dx12Wrapper::Init", "スワップチェーンの生成", result);
	// 取得した IDXGISwapChain1 を IDXGISwapChain4 にアップキャストする
	result = swapchain1.As(&_swapchain);
	if (FAILED(result)) return DebugFail("Dx12Wrapper::Init", "IDXGISwapChain4 へのキャスト", result);
	...
	if (FAILED(result)) return DebugFail("Dx12Wrapper::Init", "RTV 用ディスクリプタヒープの生成", result);

	// スワップチェーンのメモリと紐づけ
	DXGI_SWAP_CHAIN_DESC swcDesc = {};
	result = _swapchain->GetDesc(&swcDesc);
	if (FAILED(result)) return DebugFail("Dx12Wrapper::Init", "スワップチェーンの desc 取得", result);
	...
	for (int idx = 0; idx < swcDesc.BufferCount; ++idx)
	{
		...
		if (FAILED(result))
			return DebugFail("Dx12Wrapper::Init", "バックバッファの取得", result);
	...
	if (_heapForImgui == nullptr)
		return DebugFail("Dx12Wrapper::Init", "ImGui 用ディスクリプタヒープの生成");
