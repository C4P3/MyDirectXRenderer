...
)
{
	...
	if (ext == ".dds") {
		...
		// 未対応のフォーマット
		return DebugFailNull("PMDActor::LoadTextureFromFile",
			("未対応の拡張子: " + filePath).c_str());
	}

	// 読み込み失敗時は nullptr を返す
	if (FAILED(hr))
		return DebugFailNull("PMDActor::LoadTextureFromFile",
			("読み込みに失敗: " + filePath).c_str(), hr);
...
bool PMDActor::VMDMotionLoad(const char* filepath) {
	...
	if (fp == nullptr)
		return DebugFail("PMDActor::VMDMotionLoad",
			("VMD ファイルを開けない: " + strModelPath).c_str());
...
bool PMDActor::Load(const char* filepath) {
	...
	fopen_s(&fp, strModelPath.c_str(), "rb");
	if (fp == nullptr)
		return DebugFail("PMDActor::Load",
			("PMD ファイルを開けない: " + strModelPath).c_str());
	...
	if (FAILED(hr))
		return DebugFail("PMDActor::Load", "座標変換用定数バッファの生成", hr);
	...
	if (FAILED(hr))
		return DebugFail("PMDActor::Load", "座標変換用定数バッファの Map", hr);
	...
	if (FAILED(result))
		return DebugFail("PMDActor::Load", "マテリアル用定数バッファの生成", result);
	// マップマテリアルにコピー
