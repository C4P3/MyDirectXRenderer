...
- ハードコードしたラティスメッシュ（トーラス）を `roundLattice()` で Gregory パッチ列に変換
...
3. **`Update()` / `Draw()` の責務分割**
   現在は `Application::Run()` が `Scene` と各 Actor の `Update()` を直接呼び、Renderer の `Draw()` を順に呼んでいる。アニメーションを入れて Actor 側の状態が増えたときに、この平坦な呼び出しのままで良いか。

4. **リソース生成の所在**
   各 Actor が `Dx12Wrapper&` を持って自分でバッファを作っている。後述の GPU メモリ上限管理を入れるなら、生成経路を一箇所に集約する必要があるはず。
